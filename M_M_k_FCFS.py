...
service_rate = 1/6.75  # Service rate (customers/hour/server)
servers = 10        # Number of servers
simulation_time = 10710.0  # Simulation time (hours)
...
while min(arrival_time, np.min(servers_busy)) < simulation_time:
    ...
    if arrival_time < np.min(servers_busy):
        ...
        if idle_server.size > 0:
            ...
            next_service_time = expon.rvs(scale=1 / service_rate)
...
y = service_rate * np.exp(-service_rate * x)  # Calculate the corresponding y values
plt.figure()
